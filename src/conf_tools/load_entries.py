...
from .valid import check_valid_id_or_pattern
    ...
    with open(filename) as f:
        ...
        if parsed is None:
            ...
            if not isinstance(parsed, list):
                msg = "Expect the file %r to contain a list of dicts, found %s." % (
            ...
            if not all([isinstance(x, dict) for x in parsed]):
                msg = "Expect the file %r to contain a list of dicts." % filename
                raise SyntaxMistake(msg)
...
def write_entries(entries, filename_yaml):
    ...
    with open(filename_yaml, "w") as f:
        yaml.dump(entries, f, default_flow_style=False, explicit_start=True)
